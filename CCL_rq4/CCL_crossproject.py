...
def evaluate(X_test, Y_test, model1, model2):
    ...
    return acc, f1, prec, recall, roc_auc, effort, mcc
...
def main():
    ...
    file_name = 'CCL-cross.csv'
    f = open(file_name, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(f)
    csv_writer.writerow(
        ["datasets", "f1-score", "precise", "recall", "auc", "effort", "mcc"])
    ...
    for i in range(10):
        ...
        test_acc, test_f1, prec, recall, roc, effort, mcc = evaluate(X_test, Y_test, model1, model2)
        print('%s Epoch [%d/%d] Test Accuracy on the %s test: Model %.4f, f1-score: %.4f, auc: %.4f, effort-aware: %.4f ' % (
           csv_string_test, epoch + 1, args.n_epoch, len(Y_test), test_acc, test_f1, roc, effort))


        write_all.append(test_f1)
        write_all.append(prec)
        write_all.append(recall)
        write_all.append(roc)
